...
export default function BlogItem({ blog }) {
  ...
  blogContent.sort(function (b1, b2) {
  ...
  const handleExpandClick = () => {
    if (expanded === true) setDirection("rotate(0deg)");
    ...
    <>
      <Card className="border w-3/4 mt-10  bg-gray-800  rounded shadow-2xl relative">
        <CardHeader
          ...
          title={blog.heading}
          subheader={`${blog.group.coach.userName}, ${moment(
        ...
        <CardContent>
          {blogContent.map((bc) => (
            <p>
              {bc.type === "A" ? (
                <AttachmentItem attachment={bc.content} />
              ) : (
                <div>{bc.content}</div>
              )}
            </p>
      ...
      <Collapse
        className="border w-3/4 mt-3 rounded"
        in={expanded}
        timeout="auto"
        unmountOnExit
      >
